...
// Invalidate the host's registry cache via API
async function invalidateHostCache() {
  try {
    ...
    if (!response.ok) {
      // Host might not be running - this is OK, cache will be invalidated on next start
      info('Host not running, cache will be invalidated on next start');
      return;
    }
    
    const result = await response.json();
    if (result.ok) {
      info('Registry cache invalidated');
    } else {
      warn(`Cache invalidation warning: ${result.error}`);
    }
  } catch (err) {
    // Host might not be running - this is OK
    info('Host not running, cache will be invalidated on next start');
  }
}
...
async function updateSingleApp(appId) {
  ...
  // Invalidate host registry cache
...
// Local built-in templates (metadata only - full templates live in templates/ dir)
...
Commands:
  ...
  quota [app-id]                Show storage quota for an app (or all apps with --all)
  quota <app-id> --set=<mb>     Set custom quota for an app
