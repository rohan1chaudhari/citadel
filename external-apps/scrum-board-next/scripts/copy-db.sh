...
SRC_DIR="$ROOT/data/apps/scrum-board"
DST_DIR="$ROOT/external-apps/scrum-board-next/data/scrum-board"
mkdir -p "$DST_DIR"
if [[ -f "$SRC_DIR/db.sqlite" ]]; then
  ...
  echo "Copied scrum-board DB to $DST_DIR"
else
  echo "No existing scrum-board DB found at $SRC_DIR"
fi
